...
export default function HydrationDebugPanel() {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-[90]">
      <details className="bg-white rounded-lg shadow-lg border border-gray-200 max-w-2xl w-full overflow-hidden">
        <summary className="px-4 py-2 text-xs font-semibold text-gray-700 cursor-pointer bg-gray-50 border-b border-gray-200 flex items-center gap-2">
          <Activity className="w-3.5 h-3.5 text-blue-600" />
          Context Hydration Steps
          {failureStep && (
            <span className="ml-auto text-xs text-red-600 font-medium">
              Failed at: {failureStep.label}
            </span>
          )}
        </summary>
